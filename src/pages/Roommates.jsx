...
const Roommates = ({ user }) => {
  ...
  return (
    <div className="roommates-page" style={{ minHeight: "100vh", paddingBottom: "40px", backgroundColor: "#eae4f9" }}>
      <Header user={user} />

      {/* --- МІНІ-МЕНЮ (ТАБИ) --- */}
      <div className="d-flex justify-content-center mt-4 mb-2">
        <div className="bg-white p-1 rounded-pill shadow-sm d-inline-flex" style={{ border: "1px solid #eee" }}>
          <button
            onClick={() => setActiveTab("search")}
            className={`btn rounded-pill px-4 py-2 fw-bold ${activeTab === "search" ? "text-white" : "text-secondary"}`}
            style={{ backgroundColor: activeTab === "search" ? "#8a4fff" : "transparent", transition: "all 0.3s ease", border: "none" }}
          >
            Знайти співжителя
          </button>
          ...
          <button
            ...
            className={`btn rounded-pill px-4 py-2 fw-bold ${activeTab === "room" ? "text-white" : "text-secondary"}`}
            style={{ backgroundColor: activeTab === "room" ? "#8a4fff" : "transparent", transition: "all 0.3s ease", border: "none" }}
          >
            Кімната
          </button>
        </div>
      ...
      {activeTab === "room" && (
        <div className="container mt-4 text-center">
          ...
          <p className="mb-5 fw-bold text-muted">Не можете вирішити хто що робить? Нехай колесо вирішить!</p>

          <div className="card shadow-sm mb-4 border-0 rounded-4 mx-auto" style={{ maxWidth: "800px" }}>
            <div
              className="card-header bg-white d-flex justify-content-between align-items-center p-3 rounded-4"
              style={{ cursor: "pointer", borderBottom: isRoommatesOpen ? "1px solid #eee" : "none" }}
              onClick={() => setIsRoommatesOpen(!isRoommatesOpen)}
            >
              <span className="fw-bold">Сусіди по кімнаті</span>
              <button className="btn btn-sm rounded-pill" style={{ backgroundColor: "#f3f0fb", color: "#666" }}>
                {isRoommatesOpen ? `Сховати сусідів (${roommates.length})` : `Показати сусідів (${roommates.length})`}
              </button>
            </div>
            {isRoommatesOpen && (
              <div className="card-body bg-white rounded-bottom-4 overflow-visible">
                <div className="position-relative mb-3 text-start">
                  ...
                  {searchFriendTerm.length > 0 && (
                    <div className="position-absolute w-100 bg-white shadow rounded-3 mt-1" style={{ zIndex: 1000, maxHeight: "200px", overflowY: "auto" }}>
                      {filteredFriends.length > 0 ? (
                        filteredFriends.map((friend) => (
                          ...
                          >
                            <img src={friend.avatar || defaultUser} alt="avatar" className="rounded-circle me-2" style={{ width: "30px", height: "30px", objectFit: "cover" }} />
                            <span className="fw-bold" style={{ fontSize: "14px" }}>{friend.firstName} {friend.lastName}</span>
                          </div>
                        ))
                      ) : (
                        <div className="p-2 text-muted small">Друзів не знайдено або вони вже в кімнаті</div>
                      )}
                    </div>
                  )}
                </div>
                <ul className="list-group list-group-flush text-start">
                  {roommates.length === 0 && <p className="text-muted small text-center mt-2">Ви ще не додали сусідів</p>}
                  {roommates.map((rm) => (
                    <li key={rm.id} className="list-group-item d-flex justify-content-between align-items-center border rounded mb-2 py-2 px-3 bg-light">
                      <div className="d-flex align-items-center">
                        <img src={rm.avatar || defaultUser} alt="avatar" className="rounded-circle me-2" style={{ width: "35px", height: "35px", objectFit: "cover" }} />
                        <span className="fw-bold">{rm.firstName}</span>
                      </div>
                      <button className="btn border-0 text-danger p-0" onClick={() => handleDeleteRoommate(rm.id)}>
          ...
          <div className="card shadow-sm mb-5 border-0 rounded-4 mx-auto" style={{ maxWidth: "800px" }}>
            <div
              className="card-header bg-white d-flex justify-content-between align-items-center p-3 rounded-4"
              style={{ cursor: "pointer", borderBottom: isTasksOpen ? "1px solid #eee" : "none" }}
              onClick={() => setIsTasksOpen(!isTasksOpen)}
            >
              <span className="fw-bold">Завдання</span>
              <button className="btn btn-sm rounded-pill" style={{ backgroundColor: "#f3f0fb", color: "#666" }}>
                {isTasksOpen ? `Сховати завдання (${tasks.length})` : `Показати завдання (${tasks.length})`}
              </button>
            </div>
            {isTasksOpen && (
              <div className="card-body bg-white rounded-bottom-4">
                <div className="input-group mb-3">
                  ...
                  />
                  <button className="btn fw-bold text-white" style={{ backgroundColor: "#8a4fff" }} onClick={handleAddTask}>
                    <i className="bi bi-plus-lg"></i>
                  </button>
                </div>
                <ul className="list-group list-group-flush text-start">
                  {tasks.length === 0 && <p className="text-muted small text-center mt-2">Список завдань порожній</p>}
                  {tasks.map((task, index) => (
                    <li key={index} className="list-group-item d-flex justify-content-between align-items-center border rounded mb-2 py-2 px-3 bg-light">
                      <span className="fw-bold text-secondary">{task}</span>
                      <button className="btn border-0 text-danger p-0" onClick={() => handleDeleteTask(task)}>
                        <i className="bi bi-trash fs-5"></i>
                      </button>
                    </li>
                  ))}
          ...
          <div className="d-flex flex-column flex-md-row justify-content-center align-items-center gap-5 mx-auto mt-5" style={{ maxWidth: "900px" }}>
            ...
            <div className="game-controls text-center text-md-start">
              <h6 className="fw-bold mb-3">Хто буде виконувати завдання прямо зараз?</h6>
              <div className="d-flex flex-column gap-2 mb-4">
                {roommates.length === 0 && <p className="text-muted small">Додайте сусідів зверху</p>}
                {roommates.map((rm) => (
                  <button
                    key={rm.id}
                    className="btn shadow-sm fw-bold px-4 py-2 bg-white"
                    style={{
                      borderRadius: "10px",
                      color: "var(--text-main)",
                      border: winner && winner.person.id === rm.id ? "2px solid #8a4fff" : "none",
                    }}
                  >
                    {rm.firstName}
                  </button>
              ...
              <button
                ...
                style={{ backgroundColor: "#8a4fff", borderRadius: "15px", width: "100%" }}
                disabled={roommates.length === 0 || tasks.length === 0 || mustSpin}
                onClick={handleSpinClick}
              >
                {mustSpin ? "Крутимо..." : "Покрутіть колесо!"}
              </button>
              {winner && !mustSpin && (
                <div className="mt-4 p-3 bg-white rounded-4 shadow-sm text-center border" style={{ borderColor: "#20c997" }}>
                  <h5 className="mb-1 fw-bold" style={{ color: "#8a4fff" }}>Вітаємо, {winner.person.firstName}! 🎉</h5>
                  <p className="mb-0 text-secondary">Твоє завдання: <strong>{winner.task}</strong></p>
                </div>
              )}
            </div>
          </div>
      ...
      {activeTab === "search" && (
        <div className="container mt-4">
          ...
          <p className="text-muted text-center mb-5">Аналізуємо сумісність за 5 основними категоріями побуту</p>
          
          {/* Якщо користувач сам не пройшов тест */}
          {!currentUserAnswers ? (
            <div className="text-center mt-5 p-5 bg-white rounded-4 shadow-sm mx-auto" style={{ maxWidth: "600px" }}>
              <i className="bi bi-ui-checks-grid text-muted mb-3 d-block" style={{ fontSize: "3rem" }}></i>
              <h4 className="fw-bold">Ви ще не пройшли тест!</h4>
              <p className="text-secondary mb-4">Щоб ми могли підібрати вам ідеального сусіда, потрібно дізнатися про ваші побутові звички.</p>
              <button 
            ...
            <div className="text-center mt-5">
              <div className="spinner-border" style={{ color: "#8a4fff" }} role="status"></div>
              <p className="mt-2 text-muted fw-bold">Шукаємо кандидатів...</p>
            </div>
          ) : potentialMatches.length === 0 ? (
            <div className="text-center mt-5 text-muted">
              <p>Поки немає користувачів, які пройшли тест. Запросіть друзів!</p>
            </div>
          ) : (
                    ...
                    <div className="card border-0 shadow-sm rounded-4 h-100 bg-white">
                      <div className="card-body text-center p-4">
                        <img 
                          ...
                          style={{ width: "90px", height: "90px", objectFit: "cover", border: "3px solid #eae4f9" }} 
                        />
                        <h5 className="fw-bold mb-1">{candidate.firstName} {candidate.lastName}</h5>
                        <p className="text-muted small mb-4">Можливий співмешканець</p>

                        {/* Блок з 5 індикаторами */}
                        <div className="d-flex justify-content-center gap-2 mb-3">
                          {matchCategories.map((cat) => {
                            // Перевіряємо сумісність саме для цього блоку
                            const status = calculateBlockStatus(currentUserAnswers, theirAnswers, cat.ids);
                            // Зелений або червоний колір
                            const bgColor = status === "green" ? "#20c997" : "#ff6b6b";
                        ...
                        <p className="small text-muted mb-0">Наведіть на іконку для деталей</p>
                        
                        {/* Кнопка написати */}
                        <button 
                          className="btn btn-light w-100 mt-4 rounded-pill fw-bold text-secondary"
                          onClick={() => navigate("/chat", { state: { startChatWith: candidate } })} // Або може відкрити модалку профілю
                        >
